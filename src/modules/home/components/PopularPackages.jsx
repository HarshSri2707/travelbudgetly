...
const PackageCard = ({ pkg }) => (
  <div className="bg-white rounded-2xl hover:shadow-xl transition-shadow duration-300 overflow-hidden group flex flex-col w-full h-full">
    ...
    {/* Body — increased padding & spacing */}
    <div className="p-5 flex flex-col flex-1">
      <h3 className="font-bold text-gray-900 text-base mb-1.5 line-clamp-1">{pkg.name}</h3>
      <p className="text-xs text-gray-500 mb-4 leading-relaxed line-clamp-3">{pkg.description}</p>

      <div className="mb-4 space-y-1">
        <p className="text-[11px] font-semibold text-gray-700 mb-1.5">Includes</p>
        {pkg.includes.slice(0, 3).map((item, idx) => (
          <p key={idx} className="text-[11px] text-gray-400">{item}</p>
        ))}
      </div>
      ...
      <Link to={`/packages/${pkg.slug}`}>
        <button className="w-full bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white font-semibold py-3 rounded-full transition-colors duration-200 text-sm">
          Book Now
        </button>
      </Link>
    </div>
